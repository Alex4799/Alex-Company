                ...
                <div class="table-responsive table-responsive-data2" style="height: 50vh">
                           ...
                           <table class="table table-dark table-data2 text-center">
                            ...
                            <tbody>
                               ...
                               <tr class="spacer"></tr>
                               <tr class="tr-shadow ">
                                   <td class="align-middle"><a class="text-decoration-none" href="{{route('user#viewProfile',$item->user_id)}}">{{$item->user_name}}</a></td>
                                   <td class="align-middle"><a class="@if ($item->status == 0) text-warning @elseif ($item->status == 1) text-success @else text-danger @endif"
                                    href="{{route('user#orderList',$item->order_id)}}">{{$item->order_id}}</a></td>
                                   ...
                                   <td class="align-middle">
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                          <li><a class="dropdown-item" href="{{route('user#orderChangeStatus',[$item->id,0])}}">Pending</a></li>
                                          <li><a class="dropdown-item" href="{{route('user#orderChangeStatus',[$item->id,1])}}">Success</a></li>
                                          <li><a class="dropdown-item" href="{{route('user#orderChangeStatus',[$item->id,2])}}">Fail</a></li>
                                        </ul>
                                    </div>
                                   </td>
                                   <td>
                                    @if ($item->status == 2)
                                        <div>
                                            <a href="{{route('user#orderDelete',$item->id)}}" class="me-3">
